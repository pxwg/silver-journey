...
* 因为 blink-cmp 的最新设置，Copilot 的自动补全在中文环境下会输出非 UTF-8 字符，这将导致各种 LSP 全部断线。为了修正这个情况并减少输入延迟，使用 rust 编辑了一个过滤器筛出非 UTF-8 词语，响应速度毫秒级 (现在弃用，换成更为稳健的 nvim-cmp copilot source，但源代码保留以便之后使用);
